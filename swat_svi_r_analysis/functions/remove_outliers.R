...
# purpose: identify and remove statistically high outliers then give new data frame
...
remove_outliers <- function(obs_flow_input_df, kn_table) {
  # example obs_flow_input_df is the output from obs_hiflow_freq_calcs_one_rch()
  ...
  if (input_cskew >= (-0.4) & input_cskew <= (0.4)) {
    ...
    if (input_cskew < (-0.4)) {
      ...
      obs_output_df <- obs_flow_input_df %>% 
        filter(obs_max_flow_log_cms_adj > input_lowflow_limit)
      ...
      #output new df
      obs_output_df <- obs_flow_input_df %>% 
        filter(obs_max_flow_log_cms_adj < input_highflow_limit)
