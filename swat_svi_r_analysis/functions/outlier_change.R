...
# purpose: calculate % change in number of minor and major outliers between baseline and projection data
# last updated: 20171120
# author: sheila saia
# contact: ssaia [at] ncsu [dot] edu

outlier_change <- function(baseline_outlier_counts_sum, projection_outlier_counts_sum, num_yrs) {
  ...
  # this function requires the tidyverse package
  ...
  for (i in 1:num_rchs) {
    ...
    baseline_rch_temp <- baseline_outlier_counts_sum %>% 
      filter(RCH == i)
    ...
    projection_rch_temp <- projection_outlier_counts_sum %>% 
      filter(RCH == i)
